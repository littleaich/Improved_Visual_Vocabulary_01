...
% % from Oxford-102 flower dataset. It forms the bag-of-words 
...
% % kernel learning (Oxford-102 dataset) is listed as follows: 
...
for i = 1:length(tstid) 
    ...
    %     end
        ...
        if strcmp(image_version,'Images_Min_500_Div')
            f = double(func_genDivergenceDb(tstid(i)))/MAX_PIX_VAL;
            desc = modifiedBoFDivColorExtractor(f,tstid(i),'hsv',blockSize,[],[],1);
        else
            f = im2double(imread('../../Databases/Oxford/Images_Min_500/4320.jpg'));
            desc = modifiedBoFColorExtractor(f,'hsv',blockSize,[],[],1);
        end
