...
% Oxford-102 flower dataset. It extracts the HSV color 
% descriptors as the average values in consecutive non-overlapping nxn 
% blocks (n = 3 here). The processing for the color feature 
...
blockSize = 3;
alpha = 1;
ratio = [0.2,0.8,0.0]; % rgb-hsv-lab raito
...
for i = 1:numImg
    ...
    elseif strcmp(image_version,'Images_Min_500') || ...
            strcmp(image_version,'Images_Min_500_Div') || ...
            strcmp(image_version,'Images_Org_Min_500') 
        addZeros = '';
    end
    ...
    if strcmp(image_version,'Images_Min_500_Div')
        if i~=4320
            load([num2str(i),'.mat']);
            im = double(im)/MAX_PIX_VAL;
        else
            im = double(func_genDivergenceDb(i))/MAX_PIX_VAL;
        end
        desc = modifiedBoFDivColorExtractor(im,i,featName,blockSize,cform,ratio,alpha);    
    else
        f = im2double(imread([imPref,addZeros,num2str(i),'.jpg']));
        desc = modifiedBoFColorExtractor(f,featName,blockSize,cform,ratio,alpha);    
    end
    desc = single(desc);
