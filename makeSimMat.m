...
% from Oxford-102 flower dataset. It calculates the similarity
...
% dataset) is listed as follows: (1) extractColorDescriptors.m,
...
tmp = tmp/max(tmp(:));
tmp = 1-tmp;
%tmp = exp(tmp).*exp(tmp); % square exponential mapping
simMat{1} = tmp;
clear tmp;
